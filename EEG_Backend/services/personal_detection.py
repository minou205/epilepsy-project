...
TRAIN_CONFIG = dict(INTER_H=INTER_H, POSTICTAL_H=POSTICTAL_H,
                    ICTAL_STRIDE_S=ICTAL_STRIDE_S, MA_S=MA_S,
...
print(f'MODE: SEIZURE DETECTION (Ictal vs Interictal)')
print(f'Channels={N_CH} | Clip={CLIP_S}s | L={L}')
print(f'Ictal stride={ICTAL_STRIDE_S}s (overlapping) | Alarm gate={DELTA0_S}s')
print(f'WGAN seq_len={WGAN_SEQ_LEN} | Config sig: {TRAIN_CONFIG_SIG}')
...
def BPatDataset(patient_dir, use_cluster_rule=True, normalize=True):
    ...
    for edf in edfs:
        ...
        for onset, offset in szmap.get(edf.name, []):
            all_sz_onsets.append(cursor + onset)
            all_sz_spans.append((cursor + onset, cursor + offset))
        cursor += dur
    ...
    for edf in edfs:
        ...
        for onset_r, offset_r in szmap.get(edf.name, []):
            ...
            for s in range(start_samp, end_samp - CLEN + 1, ICTAL_STRIDE):
        ...
        for s in range(0, norm_data.shape[1] - CLEN, CLEN):
            ...
            for sz_on, sz_off in all_sz_spans:
                if sz_on <= t_abs <= sz_off:
                    in_ictal = True; break
            if in_ictal: continue
            c = norm_data[:, s:s + CLEN]
            if c.shape[1] == CLEN:
                all_int.append(c[None])

    if not all_ictal:
        print(f'  Skip {patient_dir.name}: no ictal clips')
        return None, None
    ...
    if not all_int:
        ...
        for edf in edfs:
            ...
            for s in range(0, norm_data.shape[1] - CLEN, CLEN):
                t_abs = abs_start + s / FS
                if all(abs(t_abs - sz) > 30 * 60 for sz in used_sz):
                    in_ictal = False
                    for sz_on, sz_off in all_sz_spans:
                        if sz_on <= t_abs <= sz_off:
                            in_ictal = True; break
    ...
    clips  = np.concatenate([ictal, int_])
    labels = np.array([1] * len(ictal) + [0] * len(int_))
    print(f'    ictal={len(ictal)} | interictal={len(int_)}')
    return clips, labels

print('Detection data loading ready')
...
def compute_gradient_penalty(discriminator, ict_feats, int_feats, dev):
    B     = min(len(ict_feats), len(int_feats))
    ict_f = ict_feats[:B].detach()
    int_f = int_feats[:B].detach()
    alpha = torch.rand(B, 1, 1, device=dev) if ict_f.dim() == 3 else torch.rand(B, 1, device=dev)
    f_hat = (alpha * int_f + (1 - alpha) * ict_f).requires_grad_(True)
...
print(f'SequenceWGANDiscriminator ready (seq_len={WGAN_SEQ_LEN})')
...
class ECLoss(nn.Module):
    ...
    def forward(self, X, y):
        ...
        with torch.no_grad():
            ict_mask = (y_col == 1).float()
            int_mask = (y_col.t() == 0).float()
            raw_sim  = Xn @ Xn.t()
            cross    = raw_sim * ict_mask * int_mask
...
print('ECLoss + CLEPLoss + FocalLoss ready')
...
def pretrain_clep(encoder, source_datasets, epochs=50, batch_size=64,
    ...
    print(f'[Pretrain] {len(source_datasets)} source patients | '
          f'{len(loader.dataset)} clips | {epochs} epochs')
    ...
    for ep in range(1, epochs + 1):
        ...
        if ep == 1 or ep % 10 == 0:
            ...
            print(f'  Epoch {ep:3d}/{epochs}  Loss={avg:.4f}  '
                  f'EC={ec_total/len(loader):.4f}  CE={ce_total/len(loader):.4f}  '
...
def finetune_best(encoder, train_dataset, val_dataset=None,
    ...
    _alpha = torch.tensor([FOCAL_ALPHA_INT, FOCAL_ALPHA_ICT], dtype=torch.float32)
    ...
    for ep in range(1, epochs + 1):
        ...
        if val_loader:
            ...
            m       = compute_metrics(np.array(all_probs), np.array(all_preds), np.array(all_true))
            val_acc = m['acc']
            val_msg = (f'  Val Acc={m["acc"]*100:.1f}%  '
...
def calibrate_threshold(encoder, val_dataset, dev=None):
    ...
    try:
        ...
        j      = tpr - fpr
        thresh = float(np.clip(thresholds[j.argmax()], 0.40, 0.70))
        m      = compute_metrics(probs, (probs >= thresh).astype(int), labels)
...
def _make_sequences(clips, seq_len=WGAN_SEQ_LEN):
    ...
    if N < seq_len:
        ...
        N = seq_len
    return np.stack([clips[i:i + seq_len] for i in range(N - seq_len + 1)])
...
def train_discriminator_wgan(encoder, discriminator, train_dataset,
    ...
    ict_clips  = all_clips[all_labels == 1]
    int_clips  = all_clips[all_labels == 0]

    if len(ict_clips) == 0 or len(int_clips) == 0:
        raise ValueError('Need both classes for WGAN training')

    ict_seqs = _make_sequences(ict_clips)
    int_seqs = _make_sequences(int_clips)
    n = min(len(ict_seqs), len(int_seqs))
    ict_seqs = ict_seqs[np.random.choice(len(ict_seqs), n, replace=False)]
    int_seqs = int_seqs[np.random.choice(len(int_seqs), n, replace=False)]

    ict_t = torch.as_tensor(ict_seqs, dtype=torch.float32)
    int_t = torch.as_tensor(int_seqs, dtype=torch.float32)
    ict_ld = DataLoader(TensorDataset(ict_t), batch_size=batch_size, shuffle=True, drop_last=True)
    ...
    discriminator.train()
    for ep in range(1, epochs + 1):
        d_ict_last = d_int_last = gp_last = None
        for (pb,), (ib,) in zip(ict_ld, int_ld):
            pb, ib = pb.to(dev), ib.to(dev)
            optimizer.zero_grad()
            with torch.no_grad():
                B, S, C, T = pb.shape
                ict_feats = encoder.encode(pb.reshape(B*S, C, T)).reshape(B, S, -1)
                int_feats = encoder.encode(ib.reshape(B*S, C, T)).reshape(B, S, -1)
            d_ict = discriminator(ict_feats)
            d_int = discriminator(int_feats)
            gp    = compute_gradient_penalty(discriminator, ict_feats, int_feats, dev)
            loss  = d_ict.mean() - d_int.mean() + lambda_gp * gp
            loss.backward()
            optimizer.step()
            d_ict_last = d_ict.mean().item()
            d_int_last = d_int.mean().item()
            gp_last    = gp.item()

        if ep % CHECK_EVERY == 0 or ep == 1:
            w_dist = d_int_last - d_ict_last
    ...
    with torch.no_grad():
        for (pb,) in DataLoader(TensorDataset(ict_t), batch_size=batch_size, shuffle=False):
            B, S, C, T = pb.shape
            feats = encoder.encode(pb.to(dev).reshape(B*S, C, T)).reshape(B, S, -1)
            ict_scores_list.append(discriminator(feats).cpu().numpy())
        for (ib,) in DataLoader(TensorDataset(int_t), batch_size=batch_size, shuffle=False):
            B, S, C, T = ib.shape
            feats = encoder.encode(ib.to(dev).reshape(B*S, C, T)).reshape(B, S, -1)
            int_scores_list.append(discriminator(feats).cpu().numpy())

    ict_scores = np.concatenate(ict_scores_list)
    int_scores = np.concatenate(int_scores_list)
    disc_calibration = float((ict_scores.mean() + int_scores.mean()) / 2.0)
    sep = int_scores.mean() - ict_scores.mean()
...
def find_hard_interictal(encoder, interictal_clips, threshold, dev, batch_size=64):
    encoder.eval()
    hard_idxs = []
    ds     = TensorDataset(torch.as_tensor(interictal_clips, dtype=torch.float32))
...
def save_patient_brain(model, discriminator, patient_id, disc_calibration=0.0,
                        ...
                        folder='patient_detector'):
    os.makedirs(folder, exist_ok=True)
    path = os.path.join(folder, f'{patient_id}_detector.pt')
    ...
    torch.save({
        ...
        'model_type'       : 'hybrid_sts_detection_v1',
    }, path)
    print(f'Detector saved → {path} (thresh={calib_thresh:.3f}, has_disc={has_disc})')
    return path
...
def train_detector(patient_id, patient_data_dir, chb_mit_dir, output_dir, tier):
    ...
    def load_csv_clips(file_list):
        ...
        for fp in file_list:
            try:
                ...
                print(f'[train_detector] Warning: skipping {fp}: {e}')
        if not all_clips:
            return np.empty((0, N_CH, CLEN), dtype=np.float32)
        return np.concatenate(all_clips, axis=0)

    ictal_clips  = load_csv_clips(ictal_files)
    normal_clips = load_csv_clips(normal_files + fp_files)

    if len(ictal_clips) < 2:
        raise ValueError(f'Not enough ictal data for {patient_id}: {len(ictal_clips)} clips')
    ...
    train_ict = (train_ict - train_ref_mu) / train_ref_std
    train_int = (train_int - train_ref_mu) / train_ref_std
    val_ict   = (val_ict - train_ref_mu) / train_ref_std
    val_int   = (val_int - train_ref_mu) / train_ref_std

    train_clips  = np.concatenate([train_ict, train_int])
    train_labels = np.array([1] * len(train_ict) + [0] * len(train_int))
    val_clips    = np.concatenate([val_ict, val_int])
    val_labels   = np.array([1] * len(val_ict) + [0] * len(val_int))
    ...
    if len(hard_int_idxs) > 10:
        ...
        adv_clips  = np.concatenate([train_ict, extra_int, train_int])
        adv_labels = np.array([1]*len(train_ict) + [0]*len(extra_int) + [0]*len(train_int))
...
if __name__ == '__main__':
  ...
  n_ict = len(ict)
  n_sz_approx = max(1, n_ict // 20)
  n_val_ict   = max(5, n_ict // n_sz_approx)
  n_val_ict   = min(n_val_ict, n_ict // 2)

  train_ict  = ict[:-n_val_ict]
  val_ict    = ict[-n_val_ict:]
  split_idx  = int(len(inte) * len(train_ict) / max(len(ict), 1))
  split_idx  = max(1, min(split_idx, len(inte) - 1))
  ...
  val_clips  = np.concatenate([val_ict, val_inte])
  val_labels = np.array([1] * len(val_ict) + [0] * len(val_inte))

  train_clips  = np.concatenate([train_ict, train_inte])
  train_labels = np.array([1] * len(train_ict) + [0] * len(train_inte))

  train_ds = EEGDataset(train_clips, train_labels)
  val_ds   = EEGDataset(val_clips,   val_labels)

  print(f'   Train: {len(train_ict)} ictal + {len(train_inte)} interictal')
  print(f'   Val:   {len(val_ict)} ictal + {len(val_inte)} interictal')
  ...
  if len(hard_int_idxs) > 10:
      ...
      adv_clips  = np.concatenate([train_ict, extra_int, train_inte])
      adv_labels = np.array([1]*len(train_ict) + [0]*len(extra_int) + [0]*len(train_inte))
      adv_ds     = EEGAugDataset(adv_clips, adv_labels, AUG_NOISE_STD, AUG_CH_DROP)
  ...
  def seizure_alarm_system(edf_file_path, patient_id,
                            brain_folder='patient_detector', plot=True):

      dev = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
      brain_path = os.path.join(brain_folder, f'{patient_id}_brain.pt')

      if not os.path.exists(brain_path):
          raise FileNotFoundError(f'No detector found for {patient_id}.')
          ...
          for start in range(0, T_total - CLEN + 1, FS):
              ...
              if discriminator is not None:
                  ...
                  if len(emb_buffer) == WGAN_SEQ_LEN:
                      ...
                      disc_int_p = float(torch.sigmoid(
                          torch.tensor(raw_d - disc_calibration)).item())
                      suppression = (1.0 - disc_int_p)

              fused_probs.append(p_ict * suppression)
      ...
      if first_alarm_sec is not None:
          ...
          print(f'  SEIZURE DETECTED')
          print(f'     Time: {first_alarm_sec}s ({a_min:.2f} minutes)')
          print(f'     Confidence: {smoothed[first_alarm_sec + DELTA0_S - 1]:.3f}')
          print(f'     Threshold: {alarm_thresh:.3f} sustained for {DELTA0_S}s')
      else:
          print(f'  STATUS: NO ACTIVE SEIZURE DETECTED')
          print(f'     Threshold w={alarm_thresh:.3f} was never sustained for {DELTA0_S}s.')
      print('=' * 60 + '\n')

      if plot:
          render_detection_plot(fused_probs, smoothed, alarm_thresh,
                                first_alarm_sec, patient_id)

      return smoothed

  def render_detection_plot(raw, smooth, thresh, alarm_sec, pid):
      t_min = np.arange(len(raw)) / 60.0
      plt.figure(figsize=(15, 5))
      plt.plot(t_min, raw, alpha=0.15, color='gray', label='Fused Raw')
      plt.plot(t_min, smooth, color='navy', lw=2, label='Smoothed Detection')
      plt.axhline(thresh, color='red', ls='--', label=f'Threshold (w={thresh:.2f})')

      if alarm_sec is not None:
          alarm_min = alarm_sec / 60
          plt.axvline(alarm_min, color='orange', lw=3, ls='-.',
                      label=f'DETECTION at {alarm_sec}s')
          above = smooth >= thresh
          plt.fill_between(t_min, 0, 1, where=above,
                           color='red', alpha=0.08, label='Active seizure zone')

      plt.title(f'Seizure Detection - Patient {pid}')
      plt.xlabel('Time (min)')
      plt.ylabel('P(ictal)')
